...
//	{ SCPI_CMD_LOCAL, "SYST:LOCAL" },
	...
	{ SCPI_CMD_GET_MEAS_POWER, "MEAS:POWER? CH%s" },
	...
	{ SCPI_CMD_GET_OVER_VOLTAGE_PROTECTION_THRESHOLD, ":SOUR:VOLT:PROT?" },
	{ SCPI_CMD_GET_OVER_CURRENT_PROTECTION_ACTIVE, ":SOUR:CURR:PROT?" },
	{ SCPI_CMD_GET_OVER_VOLTAGE_PROTECTION_ACTIVE, ":SOUR:VOLT:PROT?" },
	ALL_ZERO
};
		...
		unsigned int *num_channel_groups)
{
	...
	*/
	...
	if ((unsigned long)channel_count > ARRAY_SIZE(eez_psu_channel_names)) {
		/*
		 ...
		 * be robust.
		 */
		sr_warn("Only first %d of %lu channels of EEZ PSU are supported by the current driver", channel_count, ARRAY_SIZE(eez_psu_channel_names));
	...
	*channel_groups = g_malloc0(sizeof(struct channel_group_spec) * channel_count);
	for (i = 0; i < channel_count; i++) {
		...
		(*channels)[i].current[2] = 0.001; // programming resolution
		(*channels)[i].current[3] = 3; // spec digits
		(*channels)[i].current[4] = 3; // encoding digits
		...
		(*channels)[i].voltage[2] = 0.001; // programming resolution
		(*channels)[i].voltage[3] = 3; // spec digits
		(*channels)[i].voltage[4] = 3; // encoding digits
...
static char channel_regulation[6][5];

static int eez_update_status(const struct sr_dev_inst *sdi)
{
	...
	for(GSList *current = sdi->channel_groups; current != NULL; current = g_slist_next(current), ++i)
	{
		cg = (struct sr_channel_group *)current->data;
		ret = sr_scpi_cmd_resp(sdi, devc->device->commands,
			0, NULL, &data, G_VARIANT_TYPE_STRING, SCPI_CMD_GET_OUTPUT_REGULATION, cg->name);
		if (ret != SR_OK)
			ret = SR_ERR_DATA;
		s = g_variant_get_string(data, NULL);

		if (g_strcmp0(s, channel_regulation[i]) == 0)
			continue;

		snprintf(channel_regulation[i], 5, "%s", s);
...
SR_PRIV const struct scpi_pps pps_profiles[] = {
	 ...
	 */
	{ "Envox", "EEZ H24005 [(]Due[)]", SCPI_DIALECT_UNKNOWN, 0,
	...
	{ "Envox", "EEZ BB3 [(]STM32[)]", SCPI_DIALECT_EEZ, 0,
		ARRAY_AND_SIZE(eez_psu_devopts),
		ARRAY_AND_SIZE(eez_psu_devopts_cg),
		NULL, 0,
		NULL, 0,
		...
		.probe_channels = eez_psu_probe_channels,
		.init_acquisition = NULL,
		.update_status = eez_update_status,
	},
