...
shared_ptr<Packet> Context::create_header_packet(uint64_t start_time)
{
	auto header = g_new(struct sr_datafeed_header, 1);
	header->feed_version = 1;
	header->starttime.tv_sec = start_time / 1000;
	header->starttime.tv_usec = start_time % 1000;
...
uint64_t Header::start_time() const
{
	return (uint64_t)(_structure->starttime.tv_sec * 1000 + _structure->starttime.tv_usec);
}
